...
public class AmministratoreFacade {
	...
	public List<Amministratore> getAllAdmins() {
		...
		try {
			String query = "SELECT admin FROM Amministratore admin";
	...
	public Amministratore trovaAdminByEmailPwd(String email, String password) throws Exception {
	...
	public Amministratore getAmministratoreByID(long id) {
		...
		try {
			String query = "SELECT admin FROM Amministratore admin WHERE admin.id =: id";
			TypedQuery<Amministratore> q = em.createQuery(query, Amministratore.class);
